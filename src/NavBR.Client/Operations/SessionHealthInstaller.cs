...
internal static class SessionHealthInstaller
{
    private const string ButtonTag = "alpha12-session-health";
    ...
    private static Button CreateButton(MainWindow window)
    {
        ...
        {
            system.Children.Add(button);
            return button;
        }

        var dispatcherButton = FindButtons(window).FirstOrDefault(candidate =>
            candidate.Tag as string == "alpha12-dispatcher");
        if (dispatcherButton?.Parent is Panel panel)
        {
            var index = panel.Children.IndexOf(dispatcherButton);
    ...
    private static void ApplyLocalization(Button button) =>
        button.Content = SessionHealthWindow.MenuText();
